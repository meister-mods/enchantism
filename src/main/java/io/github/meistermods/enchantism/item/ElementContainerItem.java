...
public final class ElementContainerItem extends Item {
  ...
  public static ElementType getElement(ItemStack stack) {
    ...
    return ElementType.fromName(tag.getString(ELEMENT_TAG));
  }
  ...
  public static int getElementAmount(ItemStack stack) {
    ...
    return Math.max(0, tag.getInt(AMOUNT_TAG));
  }

  public static void setElement(ItemStack stack, ElementType element, int amount) {
    ...
    tag.putString(ELEMENT_TAG, element.getSerializedName());

    tag.putInt(AMOUNT_TAG, Math.min(amount, MAX_ELEMENT_AMOUNT));
  }

  public static boolean canAccept(ItemStack container, ElementType materialElement) {
    ...
    return storedElement == ElementType.EMPTY || storedElement == materialElement;
  }

  public static boolean addElement(ItemStack container, ElementType materialElement, int amount) {
    if (amount <= 0) {
      return false;
    }
    ...
    long newAmount = (long) currentAmount + amount;

    if (newAmount > MAX_ELEMENT_AMOUNT) {
      return false;
    }

    setElement(container, materialElement, (int) newAmount);
  ...
  public void appendHoverText(
    ...
    if (element == ElementType.EMPTY) {
      tooltip.add(
          Component.translatable("tooltip.enchantism.element_container.empty")
    ...
    tooltip.add(
        Component.translatable(
                "tooltip.enchantism.element_container.element", element.getDisplayName())
            .withStyle(ChatFormatting.AQUA));

    String formattedAmount = String.format("%,d", amount);

    String formattedMaximum = String.format("%,d", MAX_ELEMENT_AMOUNT);
